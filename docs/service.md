...
거래 추가·검증·최신순 목록·검색·월별 요약·수정·삭제·CSV 입출력, 예산 설정·조회와 카테고리 추가·조회·삭제를 구현했습니다.
...
| update_transaction | 구현: id 존재 확인, 제공된 필드만 검증·반영하고 안전한 재작성 요청 |
| delete_transaction | 구현: id 존재 확인 후 해당 거래를 제외한 안전한 재작성 요청 |
| import_csv | 구현: CSV 헤더·행 검증, 새 id 생성, 성공·건너뜀 건수 반환 |
| export_csv | 구현: 월 또는 시작·종료일 조건 검증, CSV 생성과 처리 건수 반환 |
...
`update_transaction`은 원본 거래의 복사본에 요청받은 필드만 반영한 뒤 기존 거래 검증을 재사용합니다.
수정 옵션이 없거나 ID가 없거나 새 값이 잘못되면 오류를 내고 원본 거래 파일을 유지합니다.
`delete_transaction`은 거래를 한 건씩 읽으며 대상 ID만 제외해 같은 안전한 재작성 메서드로 전달합니다.
대상 ID가 없으면 오류를 발생시켜 임시 파일이 원본을 교체하지 못하게 합니다.
CSV는 UTF-8과 헤더를 사용하고 tags는 쉼표 구분 문자열로 변환합니다.
`import_csv`는 헤더가 틀리면 파일 전체를 거부합니다. 헤더가 정상이라면 각 행에 기존 거래 검증을 적용하고,
정상 행은 새 ID로 저장하며 잘못된 행은 건너뜁니다. 반환 튜플은 `(성공 건수, 건너뜀 건수)`입니다.
`export_csv`는 `--month`와 `--from/--to` 중 한 방식만 허용하고, 조건에 맞는 거래를 최신순으로 기록합니다.
조건에 맞는 거래가 없어도 헤더가 있는 CSV를 만들고 처리 건수 0을 반환합니다.
