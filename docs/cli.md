...
현재 `add`, `list --limit`, `search`, `summary`, `budget set/show`, `category add/list/remove`가 연결되어 있습니다. 다른 명령 함수는 미구현입니다.
...
실제 명령 예시는 README에 있으며 `add`, `list`, `search`, `summary`, `budget`, `category`가 동작합니다.
